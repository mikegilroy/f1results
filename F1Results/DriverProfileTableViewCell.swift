...
class DriverProfileTableViewCell: UITableViewCell {
    ...
    func updateCellWithRaceResult(raceResult: RaceResult) {
        ...
        if raceResult.status == "Finished" {
            ...
            case 1, 21:
                self.finalPositionLabel.text = "\(raceResult.position)st"
            case 2, 22:
                self.finalPositionLabel.text = "\(raceResult.position)nd"
            case 3, 23:
                self.finalPositionLabel.text = "\(raceResult.position)rd"
            default:
                self.finalPositionLabel.text = "\(raceResult.position)th"
            }
        } else {
            self.finalPositionLabel.text = "DNF"
        }
    ...
    func updateCellWithDriverStanding(driverStanding: DriverStanding) {
        ...
        case 1, 21:
            self.finalPositionLabel.text = "\(driverStanding.position)st"
        case 2, 22:
            self.finalPositionLabel.text = "\(driverStanding.position)nd"
        case 3, 23:
            self.finalPositionLabel.text = "\(driverStanding.position)rd"
        default:
            self.finalPositionLabel.text = "\(driverStanding.position)th"
        }
